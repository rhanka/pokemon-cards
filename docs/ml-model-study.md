...
Verified on 24 July 2026. CardScope does not call a metered general-purpose
Vision API. OCR is not accepted as the product's card-identification method.
The target is a specialised ONNX visual-retrieval model in a browser Web Worker,
once a corpus and its derived artefacts pass the release gates.
...
| [`TheFusion21/PokemonCards`](https://huggingface.co/datasets/TheFusion21/PokemonCards)                           | Roughly 13k English card rows sourced from `images.pokemontcg.io`; Hugging Face metadata declares CC-BY-NC-4.0, while the dataset card leaves upstream curation/licensing sections incomplete.                                                              | Owner-approved for a **local non-commercial experiment** through the bounded TypeScript intake. It remains blocked from public model/index distribution because upstream artwork authority is unverified. |
...
“Downloadable” is not the same as cleared for a public model. Every image
source must pass `ml/schemas/rights-manifest.schema.json`; commercial and
non-commercial experiment permissions are separate, and publication also
requires explicit model redistribution plus verified upstream rights.

## Selected technical path

The first useful visual version deliberately uses the smallest defensible path:
...
3. a lazily loaded ONNX INT8 encoder in a Web Worker using
   `onnxruntime-web` WASM/SIMD;
4. a normalized 128D embedding sent to the TypeScript API for a bounded
   top-five lookup in a compact Int8 index, followed by abstention and human
   confirmation.

The server performs no image inference in the target path, so it remains
compatible with the 20m CPU Kubernetes request. A legacy OCR measurement is
kept only as historical capacity evidence; it is not an activation fallback.
...
A public corpus can combine only sources that pass legal review:
...
User consent to upload a photograph is not assumed to settle every artwork
right. The manifest records the rights holder, legal basis, terms version,
acquisition date, content hash, permitted operations, and removal path. No
source image or trained weight is committed to this repository. The current
local `TheFusion21/PokemonCards` sample may be used only with
`train-noncommercial-experiment`; it is deliberately refused by
`publish-model-noncommercial`.
...
- compressed model/index/WASM/mapping package at most 12 MiB, Worker memory at
  most 100 MiB, and browser latency measured on two target phones.

Until then, the shipped path is honest: manual catalogue search and a
top-candidate screen. The chosen candidate supplies the printed language; the
user resolves an ambiguous printing and supplies finish and condition.
CardScope never claims authentication or automated grading.
