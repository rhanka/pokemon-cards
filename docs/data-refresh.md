...
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [TCGdex cards database](https://github.com/tcgdex/cards-database) and [API](https://tcgdex.dev/) | Primary multilingual catalogue and optional Cardmarket/TCGplayer quote adapter | Commit `de58b67397b3c8d829e9cb7f4eba01a7a7346235` carries an explicit MIT licence; the public [market integration page](https://tcgdex.dev/markets-prices) says prices are available for applications without an API key; live `base1-58` returned timestamped EUR/USD fields | Catalogue metadata enabled with attribution; neither the database licence nor the integration page expressly grants commercial redistribution of upstream marketplace prices, artwork, logos, or trademarks |
...
`PokemonTCG/pokemon-tcg-data` has no repository licence as of the study date. It must not be treated as commercially redistributable merely because it is on GitHub. TCGdex's public pricing documentation is positive product evidence, but activation still requires a recorded commercial redistribution permission or an owner-approved link-out-only presentation that does not copy the quote feed.
...
| Switch                        | Library default | POC     | Required approval                                                     |
| ----------------------------- | --------------- | ------- | --------------------------------------------------------------------- |
| `TCGDEX_CATALOG_ENABLED`      | `false`         | `true`  | Approved for metadata under the recorded MIT revision and attribution |
| `POKEMON_TCG_CATALOG_ENABLED` | `false`         | `false` | Commercial catalogue access/reuse decision for Pokémon TCG API        |
| `CARD_IMAGES_ENABLED`         | `false`         | `false` | Artwork/image delivery and cache rights for every enabled source      |
| `MARKET_QUOTES_ENABLED`       | `false`         | `false` | Price-feed access, reuse, attribution, and freshness terms            |

A disabled catalogue is never called and its cached entries are not returned. Cache keys include the complete source/image/quote policy, so enabling or disabling one data class cannot expose a response produced under another policy. Image URLs and quotes are removed from responses unless their own switches are enabled. During recognition, only the bounded name/collector-number query derived from OCR is sent to TCGdex over HTTPS; the photo and raw OCR are not sent, while the provider may retain request logs under its own policy. TCGdex attribution is reproduced in `THIRD_PARTY_NOTICES.md`.

## Current MVP refresh

The implemented MVP refreshes only on a bounded user query: it uses a shared server-side TTL cache with entry and byte budgets, per-client and global rate limits, a concurrency cap, upstream timeout/backoff, a hard byte budget before JSON parsing (`CATALOG_MAX_RESPONSE_BYTES`, 2 MiB by default and never above 16 MiB), at most 50 upstream cards per response, and bounded repeated/text/URL fields. It does not yet run the portfolio-aware scheduler described below. Market quotes are stripped (`MARKET_QUOTES_ENABLED=false`) and may only be enabled after the relevant commercial access and reuse rights are recorded. A quote whose upstream observation time is absent remains explicitly unknown; retrieval time is not presented as market observation time.
