<script lang="ts">
  ...
  async function updateFinish(
    ...
    const quotes =
      holding.card.quotes ?? (holding.card.quote ? [holding.card.quote] : []);
...
<section class="page collection-page" aria-labelledby="collection-title">
  <header class="page-heading">
    <div>
      ...
      <p>
        {translate(locale, "collection.subtitle", {
          cards: totals.cards,
  ...
  <div class="value-hero">
        ...
        {#each totals.currencies as total (total.currency)}
          <article>
            ...
            <div class="value-range">
              ...
              <b
                >{formatOptionalMoney(locale, total.low, total.currency)} – {formatOptionalMoney(
                  locale,
                  total.high,
                  total.currency,
                )}</b
            ...
            <div class="value-grid">
              <div>
                ...
                {#if total.costCoverage === "partial"}<small
                    >{translate(locale, "collection.costPartial")}</small
                  >{/if}
              </div>
              <div>
                <span>{translate(locale, "collection.net")}</span><b
                  class:negative={total.net !== null && total.net < 0}
    ...
    {:else}
      <strong class="no-values"
        >{translate(locale, "collection.noValues")}</strong
      >
    {/if}
  </div>
          ...
          <div class="holding-main">
            <div class="card-thumbnail">
              ...
              {:else}
                <Icon name="image" />
              {/if}
              {#if holding.quantity > 1}<span class="duplicate-badge"
                  >×{holding.quantity}</span
                >{/if}
            </div>
            <div class="holding-copy">
              <span
                >{holding.card.setName ?? "—"} · {holding.card.printedNumber ??
                  holding.card.number ??
                  "—"}</span
              >
      ...
      <ol>
        {#each snapshot.activities.slice(0, 8) as activity (activity.id)}
          <li>
            ...
            <div>
              <strong>{activity.cardName}</strong><small
                >{new Intl.DateTimeFormat(locale === "fr" ? "fr-CA" : "en-CA", {
                  dateStyle: "medium",
                  timeStyle: "short",
                }).format(new Date(activity.occurredAt))}</small
              >
            </div>
            {#if activity.quantityDelta}<b
                >{activity.quantityDelta > 0
                  ? "+"
                  : ""}{activity.quantityDelta}</b
              >{/if}
          </li>
        {/each}
      </ol>
...
<style>
  ...
  .currency-market strong {
    font: 750 2.15rem/1.15 var(--font-display);
  ...
  .quantity-controls {
    grid-column: 1/-1;
    display: grid;
    grid-template-columns: 3rem 2rem 3rem 1fr;
    align-items: center;
  ...
  .empty-state h2 {
    max-width: 20rem;
    margin: 0;
    font: 650 1rem/1.4 var(--font-display);
  }
